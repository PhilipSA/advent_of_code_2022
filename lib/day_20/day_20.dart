...
void day20(IResultReporter resultReporter) {
  ...
  final part2 = _day20Read(fileLines, true);
...
int _day20Read(List<String> fileLines, bool part2) {
  var numbers = fileLines
      .mapIndexed(
        (index, e) => _NumberWrapper(
          index,
          int.parse(e) * (part2 ? 811589153 : 1),
        ),
      )
      .toList();
  final numbersCopy = numbers.toList();
  final numberOfMixes = part2 ? 10 : 1;
  ...
  for (var i = 0; i < numberOfMixes; ++i) {
    for (final number in numbersCopy) {
      ...
      final index = numbersReversed
          .indexWhere((element) =>
      element.originalIndex == number.originalIndex);
  ...
  return numbers
          .elementAt((number0Index + 1000) % (numbersCopy.length - 1))
          .number +
      numbers
          .elementAt((number0Index + 2000) % (numbersCopy.length - 1))
          .number +
      numbers
          .elementAt((number0Index + 3000) % (numbersCopy.length - 1))
          .number;
}
